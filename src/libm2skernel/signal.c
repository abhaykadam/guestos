...
////////void (*function_signal_a)();
//void (*function_signal_b)();
void myhandler_SIGFPE()
{
	printf("\n Inside handler for SIGFPE!!!!");
}
void myhandler_SIGUSR1()
{
	printf("\n Inside handler for SIGUSR1!!!!");
}
void myhandler_SIGUSR2()
{
	printf("\n Inside handler for SIGUSR2!!!!!");
}
struct signal_handlers_t *install_signal_handlers(void)
{
	struct signal_handlers_t *myhandler;
	myhandler=calloc(1, sizeof(struct signal_handlers_t));
	//function_signal_a=mytest;
//	myhandler->sigaction[11].handler=function_signal;
//	myhandler->sigaction[11].handler=mytest;
	myhandler->sigaction[7].sig_handler_pointer=myhandler_SIGFPE;
	myhandler->sigaction[9].sig_handler_pointer=myhandler_SIGUSR1;
	myhandler->sigaction[11].sig_handler_pointer=myhandler_SIGUSR2;
	return myhandler;
}
...
void signal_handler_run(struct ctx_t *ctx, int sig)
{
	...
	printf("\n inside signal_handler_run ****** for signal %d",sig);
//////////ctx->signal_handlers->sigaction[12].handler=mytest;
////	handler = mem_map_space(ctx->mem, MEM_PAGESIZE, MEM_PAGESIZE);
/////	mem_map(ctx->mem, ctx->signal_handlers->sigaction[sig-1].handler, MEM_PAGESIZE, mem_access_exec | mem_access_init);
/////	syscall_debug("  return code of signal handler allocated at 0x%x\n", ctx->signal_masks->pretcode);
////	mem_access(ctx->mem, ctx->signal_masks->pretcode, sizeof(signal_retcode), signal_retcode, mem_access_init);
...
//	function_signal_b=function_signal_a;
//	if (!handler)
	if(!ctx->signal_handlers->sigaction[sig-1].sig_handler_pointer)
		fatal("signal_handler_run: invalid signal handler");
	//handler();
	//ctx->regs->eip = handler;
//	function_signal_b();
	ctx->signal_handlers->sigaction[sig-1].sig_handler_pointer();
	//handler();
}
